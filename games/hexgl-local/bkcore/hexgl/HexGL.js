...
bkcore.hexgl.HexGL.prototype.displayScore = function(f, l)
{
	...
	if(this.gameover !== null)
	{
		...
		// Notify the parent wrapper that the race is complete.
		// Only fired for a genuine finish (not ship-destroyed).
		if(this.gameplay.result === this.gameplay.results.FINISH)
		{
			window.parent.postMessage(
				{ type: 'hexgl-race-complete', time: f },
	...
	if(this.gameplay.result == this.gameplay.results.FINISH)
	{
		// Notify the parent wrapper (belt-and-suspenders; primary path is the gameover block above).
		window.parent.postMessage(
			{ type: 'hexgl-race-complete', time: f },
			window.location.origin
		);
