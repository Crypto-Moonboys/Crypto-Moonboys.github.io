...
export function createSignalDuelSystem({ sendChallenge, sendAccept, sendAction } = {}) {
  ...
  function resolveLocalTurn({ actionA = 'fight', actionB = 'fight', timingA = 0.5, timingB = 0.5, levelA = 1, levelB = 1, rarityA = 'common', rarityB = 'common' } = {}, rng = Math.random) {
    ...
    const dodgeA = actionA === 'run' && rng() < 0.5;
    const dodgeB = actionB === 'run' && rng() < 0.5;

    const rawA = baseDamage(levelA, rarityA, timingA, state.energyA, rng) * nextAttackBoostA * jackpotA.mult;
    const rawB = baseDamage(levelB, rarityB, timingB, state.energyB, rng) * nextAttackBoostB * jackpotB.mult;
    ...
    state.energyA = clamp(state.energyA - costA + TURN_REGEN, 0, 100);
    state.energyB = clamp(state.energyB - costB + TURN_REGEN, 0, 100);

    const samInterference = rng() < 0.02;
