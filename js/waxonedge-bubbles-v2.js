...
(function () {
  ...
  function applyMetricCapabilities(payload) {
    ...
    document.querySelectorAll('[data-woe-timeframe]').forEach(function (button) {
      ...
      button.disabled = button.hidden;
      button.classList.toggle('is-active', timeframe === state.timeframe && !button.hidden);
    });
    if (!timeframeAllowed(state.timeframe)) state.timeframe = '24h';
    document.querySelectorAll('[data-woe-timeframe]').forEach(function (button) {
      button.classList.toggle('is-active', (button.getAttribute('data-woe-timeframe') || '') === state.timeframe && !button.hidden);
    });
  }
