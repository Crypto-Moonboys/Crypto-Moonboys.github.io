...
/* ── ENTITY MAP ────────────────────────────────────────────────────────────
   Loaded lazily from js/entity-map.json (auto-generated by
   scripts/generate-entity-map.js).  Kept as a keyed object for O(1) look-ups
   by entity_id.  Consumers should call loadEntityMap() before accessing it.
   Search and linking code may use this for entity-aware boosting in future;
   for now it is loaded once and stored, but not yet wired into search scoring.
   ─────────────────────────────────────────────────────────────────────── */
...
function getEntityMapUrl() {
  ...
  for (const s of scripts) {
    if (/\/js\/wiki\.js([?#]|$)/.test(s.src)) {
      return s.src.replace(/\/js\/wiki\.js([?#].*)?$/, '/js/entity-map.json');
    }
  }
  return '/js/entity-map.json';
}

async function loadEntityMap() {
  if (ENTITY_MAP !== null) return ENTITY_MAP; // already loaded (or already failed)
  const url = getEntityMapUrl();
  try {
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const arr = await res.json();
    // Index by entity_id for fast look-ups
    ENTITY_MAP = {};
    if (Array.isArray(arr)) {
      arr.forEach(record => { if (record.entity_id) ENTITY_MAP[record.entity_id] = record; });
    }
  } catch (err) {
    console.warn(`[wiki] Failed to load ${url} — entity-aware features unavailable.`, err);
    ENTITY_MAP = {};
  }
  return ENTITY_MAP;
}
