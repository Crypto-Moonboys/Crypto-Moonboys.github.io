...
function adapterUpdate(context, dt) {
  ...
  // POST-RUN LOOP AUDIT — Snake Run
  //
  // Game-over detection:  Game loop tick checks state.gameOver && !state.submitted
  //   each frame.  state.gameOver is set by killSnake() when the snake collides
  //   with a wall, itself, or a lethal enemy.
  //
  // Score submission:     submitScore(ArcadeSync.getPlayer(), score, GAME_ID).catch(()=>{})
  //   — fire-and-forget from a synchronous game-loop tick (cannot await here).
