...
(function () {
  ...
  function cacheMatchesContext(state, cacheKey, context) {
    return !!(
      state &&
      context &&
      cacheKey &&
      cacheKey === context.key &&
      state.utc_day === currentUtcDay()
    );
  }
  ...
  async function getState(options) {
    ...
    if (!opts.force && !_invalidatedReason && cacheMatchesContext(_state, _cacheKey, context)) {
