  ...
  function applyDailyLoopStateToRail(shared, loopState) {
    ...
    if (isSourceLiveish(loopState, 'daily_missions')) {
      ...
      shared.dailyState = {
        ...
        missed_events_today: loopState.missed_opportunities && loopState.missed_opportunities.total_today != null
          ? Math.max(0, Math.floor(Number(loopState.missed_opportunities.total_today) || 0))
          : 0,
        missed_events_all_time: loopState.missed_opportunities && loopState.missed_opportunities.total_all_time != null
          ? Math.max(0, Math.floor(Number(loopState.missed_opportunities.total_all_time) || 0))
          : 0,
      };
      shared.dailyCounts = {
        completed: completed,
        missed: loopState.missed_opportunities && loopState.missed_opportunities.total_today != null
          ? Math.max(0, Math.floor(Number(loopState.missed_opportunities.total_today) || 0))
          : 0,
      };
    ...
    if (loopWtf) shared.wtfState = loopWtf;
