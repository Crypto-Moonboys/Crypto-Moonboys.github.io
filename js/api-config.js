...
(function () {
  ...
  function installWaxSearchIndexOverlay() {
    if (window.__MOONBOYS_WAX_SEARCH_INDEX_OVERLAY_BOUND) return;
    if (typeof window.fetch !== 'function' || typeof window.Response !== 'function') return;
    window.__MOONBOYS_WAX_SEARCH_INDEX_OVERLAY_BOUND = true;

    var nativeFetch = window.fetch.bind(window);
    var wikiIndexPattern = /(?:^|\/)js\/wiki-index\.json(?:[?#].*)?$/;

    function shouldOverlayWikiIndex(input) {
      var raw = typeof input === 'string' ? input : (input && input.url) || '';
      if (!raw) return false;
      var clean = String(raw).split('#')[0];
      return wikiIndexPattern.test(clean);
    }

    function getSupplementalUrl(sourceUrl) {
      var raw = String(sourceUrl || '');
      var suffix = 'wiki-index-wax-expansion.json';
      if (!raw) return '/js/' + suffix;
      return raw.replace(/wiki-index\.json([?#].*)?$/, suffix);
    }

    function mergeIndexEntries(baseEntries, waxEntries) {
      var byUrl = new Map();
      if (Array.isArray(baseEntries)) {
        baseEntries.forEach(function (entry) {
          if (entry && entry.url && entry.url !== '/wiki/index.html') byUrl.set(entry.url, entry);
        });
      }
      if (Array.isArray(waxEntries)) {
        waxEntries.forEach(function (entry) {
          if (entry && entry.url && entry.url !== '/wiki/index.html') byUrl.set(entry.url, entry);
        });
      }
    ...
    window.fetch = function moonboysFetchWithWaxSearchIndex(input, init) {
      if (!shouldOverlayWikiIndex(input)) return nativeFetch(input, init);

      var sourceUrl = typeof input === 'string' ? input : (input && input.url) || '';
      return nativeFetch(input, init).then(function (response) {
        var clonedResponse = response.clone();
        return clonedResponse.json().then(function (baseEntries) {
          return nativeFetch(getSupplementalUrl(sourceUrl), { cache: 'no-store' })
            .then(function (waxResponse) {
              if (!waxResponse.ok) return [];
              return waxResponse.json();
            })
            .catch(function () { return []; })
            .then(function (waxEntries) {
              var headers = new Headers(response.headers);
              headers.set('content-type', 'application/json; charset=utf-8');
              return new Response(JSON.stringify(mergeIndexEntries(baseEntries, waxEntries)), {
