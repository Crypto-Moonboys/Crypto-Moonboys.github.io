...
function detectGameId() {
  ...
  if (path.includes('hexgl')) return 'hexgl';
  return 'global';
}

function getRotationGames() {
  return ['snake', 'crystal', 'btqm', 'invaders', 'pacchain', 'breakout', 'tetris', 'asteroid-fork', 'hexgl'];
}
...
function ensureUi() {
  ...
  style.textContent = `
    ...
    #arcade-retention-mission-chip{position:fixed;right:16px;bottom:80px;top:auto;background:rgba(247,201,72,.14);border:1px solid rgba(247,201,72,.62);color:#fff;border-radius:999px;padding:7px 12px;font-size:.68rem;font-weight:800;letter-spacing:.02em;max-width:280px;line-height:1.2}
