...
(function () {
  ...
  function buildCardHtml(icon, symbol, label, id) {
    return '<div class="price-card price-card--loading" data-asset-id="' + id + '">' +
      '<div class="price-card-header">' +
        '<span class="price-card-icon" aria-hidden="true">' + icon + '</span>' +
        '<div>' +
          '<div class="price-card-symbol">' + symbol + '</div>' +
          '<div class="price-card-label">' + label + '</div>' +
        '</div>' +
      '</div>' +
  ...
  function renderSkeletons(container) {
    var cg = ASSETS.map(function (a) {
      return buildCardHtml(a.icon, a.symbol, a.label, a.id);
    });
    container.innerHTML = cg.join('');
  }
