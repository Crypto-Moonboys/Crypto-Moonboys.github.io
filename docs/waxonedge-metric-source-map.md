...
Status: PR implementation map for backend-truth metrics. This file documents the metric source of truth before UI work.
...
| pair TVL/liquidity contribution | Reserves valued in WAX/USD | `/token/:contract/:symbol/pairs` contribution fields | implemented in this PR | yes after this PR | pair WAX contribution = valued reserve total for that indexed pair | none for supported one-hop price map | test contribution fields exist |
...
- Market-cap primary scanner mode is removed until `circulating_supply` and `market_cap_*` are real.
- 7D and 30D scanner controls are removed until real indexed history populates 7d/30d volume fields.
...
- Bubble size continues to use selected real metric values from the backend; metric modes filter out records without that metric rather than substituting unrelated fields.
...
- **Worker or VPS full supply sweep:** this PR rotates WAX RPC `get_currency_stats` across all indexed-pair tokens with a bounded cursor. A VPS sweep is required if production token count exceeds Worker budgets and immediate full coverage is needed.
