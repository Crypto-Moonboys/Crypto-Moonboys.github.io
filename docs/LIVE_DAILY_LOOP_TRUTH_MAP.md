...
- Daily-loop-owned right-rail fields must not be overwritten by legacy local cache or older per-widget fetch results.
- auth_required cache is not equivalent to fresh linked auth.
- `getState()` must re-check the current auth context before serving linked cached state.
- Cache hits must match the exact UTC day and auth mode key: anonymous, `linked:auth_required`, and fresh `linked:<telegram_id>` are separate states.
