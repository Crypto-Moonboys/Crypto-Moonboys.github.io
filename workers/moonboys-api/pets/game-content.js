...
export const PET_REGION_LORE = deepFreeze({
  ...
  neon_rooftops: { title: 'Neon Rooftops', strapline: 'Above the cameras, the skyline becomes a racetrack.', lore: 'Courier crews jump transmitter towers while the Skyline Hunter follows every stolen signal.', status: 'building' },
  rugpull_mines: { title: 'Rugpull Mines', strapline: 'The old promises collapsed. The crystals remained.', lore: 'Abandoned rigs still chew through unstable tunnels, trapping miners beneath the Drill Beast’s territory.', status: 'building' },
  blockchain_sewers: { title: 'Blockchain Sewers', strapline: 'Every discarded transaction leaves a shadow.', lore: 'Toxic data flows through hidden nodes where leech swarms feed and the Sewer Validator decides what survives.', status: 'building' },
  kaiju_district: { title: 'Kaiju District', strapline: 'The evacuation sirens never fully stop.', lore: 'Collapsed arcades and giant footprints mark a district defended by recovery crews against the District Destroyer.', status: 'building' },
  moon_citadel: { title: 'Moon Citadel', strapline: 'Only a complete legend can enter the archive.', lore: 'Zero-gravity trials guard the royal record while Moon Warlord Prime waits above the city it intends to own.', status: 'building' },
});
...
export function buildPetRegionDirectory(level = 1, mastery = {}) {
  ...
  return Object.entries(PET_RUN_REGIONS).map(([key, gate]) => {
    ...
    const requirementsMet = petLevel >= gate.min_level && masteryXp >= gate.mastery_required;
    ...
    return {
      ...
      lock_reason: petLevel < gate.min_level ? `Reach Level ${gate.min_level}`
        : masteryXp < gate.mastery_required ? `Build ${gate.mastery_required} region mastery`
