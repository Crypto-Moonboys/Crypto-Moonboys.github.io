...
async function processPetJob(db, telegramId, jobKeyRaw, options = {}) {
  ...
  const factionRow = await db.prepare('SELECT faction FROM telegram_progression WHERE telegram_id = ?').bind(telegramId).first().catch(() => null);
...
function serializePetMiniAppArenaBattle(battle) {
  ...
  return {
    ...
    player_hp: Number(battle.player1_hp ?? PET_ARENA_MAX_HP),
    opponent_hp: Number(battle.player2_hp ?? PET_ARENA_MAX_HP),
    player_special: Number(battle.player1_special || 0),
    opponent_special: Number(battle.player2_special || 0),
    player: safeParsePetArenaSnapshot(battle.player1_pet_snapshot_json),
    opponent: safeParsePetArenaSnapshot(battle.player2_pet_snapshot_json),
    last_round: safeParsePetArenaSnapshot(battle.last_round_log_json),
    winner: battle.winner_telegram_id || null,
  };
}
...
async function buildPetMiniAppState(db, telegramId, botToken) {
  ...
  const liveNext = affordableUpgrade
    ? { key: 'gear_upgrade', title: `Upgrade ${String(affordableUpgrade.item_key).replaceAll('_', ' ')}`, detail: `Spend ${Object.entries(affordableUpgrade.cost).map(([key, value]) => `${value} ${key.replaceAll('_', ' ')}`).join(' + ')} to reach gear level ${affordableUpgrade.target_level}.`, action: 'gear_upgrade' }
    : availableDistrict
      ? { key: 'district_mission', title: `Build ${availableDistrict.title} mastery`, detail: `Spend 10 Energy for +25 district mastery, XP, Gold and ${availableDistrict.reward_focus.map((key) => key.replaceAll('_', ' ')).join(' / ')}.`, action: 'district_mission' }
      : null;
...
async function processPetMiniAppAction(db, telegramId, user, body, botToken) {
  ...
  if (action === 'district_mission') {
    ...
    const faction = await db.prepare('SELECT faction FROM telegram_progression WHERE telegram_id=?').bind(telegramId).first().catch(() => null);
  ...
  if (action === 'gear_upgrade') return processPetEquipmentUpgrade(db, telegramId, body.item_key);
  if (action === 'cosmetic_unlock') return processPetCosmeticUnlock(db, telegramId, body.cosmetic_key);
  if (action === 'prestige') {
    ...
    return processPetPrestige(db, telegramId, live);
  }
  ...
  if (action === 'arena_start') {
    ...
    const miniChatId = `mini:${telegramId}`;
    const active = await hasActivePetArenaBattle(db, miniChatId, telegramId);
  ...
  if (action === 'kaiju_start') {
    ...
    const active = await getActivePetKaijuMatch(db, miniChatId);
    if (active && String(active.player1_telegram_id) === telegramId) return { accepted: true, reason: 'kaiju_resumed', match: active };
  ...
  if (action === 'kaiju_card') {
    ...
    if (!match || String(match.chat_id) !== `mini:kaiju:${telegramId}` || String(match.player1_telegram_id) !== telegramId) {
    ...
    return finishPetKaijuMatch(db, await getPetKaijuMatch(db, match.match_id));
  }
...
async function cmdGkHelp(tok, chatId) {
  ...
  await sendTelegramMessage(tok, chatId,
    ...
    `/petcoach — Get the best next move and unlock guidance\n` +
