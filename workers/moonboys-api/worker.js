...
function buildPetArenaMoveReplyMarkup(battleId) { return { inline_keyboard: [[{ text: 'Attack Head', callback_data: `pet:arena:mv:${battleId}:ah` }, { text: 'Attack Body', callback_data: `pet:arena:mv:${battleId}:ab` }], [{ text: 'Block Head', callback_data: `pet:arena:mv:${battleId}:bh` }, { text: 'Block Body', callback_data: `pet:arena:mv:${battleId}:bb` }], [{ text: 'Charge Special', callback_data: `pet:arena:mv:${battleId}:ch` }, { text: 'Special Move', callback_data: `pet:arena:mv:${battleId}:sp` }], [{ text: 'Forfeit', callback_data: `pet:arena:ff:${battleId}` }]] }; }
function parsePetArenaCallbackPayload(payload) {
  ...
  const move = text.match(/^arena:mv:(a-[a-f0-9]{10}):(ah|ab|bh|bb|ch|sp)$/);
  if (move) return `mv:${move[1]}:${move[2]}`;
...
async function readyPetArenaBattle(db, battle, telegramId) {
  ...
  await db.prepare(`UPDATE telegram_pet_arena_battles SET ${column}=COALESCE(${column}, CURRENT_TIMESTAMP), status='active' WHERE battle_id=? AND status IN ('readying','active')`).bind(battle.battle_id).run();
  const updated = await getPetArenaBattle(db, battle.battle_id);
  await ensurePetArenaRound(db, battle.battle_id, Number(updated?.current_round || 1));
  if (updated?.player1_ready_at && updated?.player2_ready_at) return { accepted:true, reason:'round_prompt', battle: updated };
...
async function applyPetArenaMove(db, battle, telegramId, move) {
  const isP1 = String(battle.player1_telegram_id) === String(telegramId), isP2 = String(battle.player2_telegram_id) === String(telegramId);
  if (!isP1 && !isP2) return { accepted:false, reason:'not_participant', battle };
  if (String(battle.status) !== 'active') return { accepted:false, reason:'battle_not_active', battle };
  const roundNumber = Number(battle.current_round || 1); await ensurePetArenaRound(db, battle.battle_id, roundNumber);
  ...
  if (!round.player1_move || !round.player2_move) return { accepted:true, reason:'waiting_for_opponent', battle: await getPetArenaBattle(db, battle.battle_id) };
...
async function cmdPetArena(db, tok, chatId, telegramId, argStr = '', chatType = '') {
  ...
  if (arg.startsWith('mv:')) { const [, battleId, move] = arg.match(/^mv:(a-[a-f0-9]{10}):(ah|ab|bh|bb|ch|sp)$/) || []; const battle = await getPetArenaBattle(db, battleId); if (!battle || String(battle.chat_id) !== String(chatId) || Number(battle.current_round || 1) < 1 || String(battle.status) !== 'active') { await sendTelegramMessage(tok, chatId, 'Stale Pet Arena move. Choose from the latest round prompt.'); return; } const applied = await applyPetArenaMove(db, battle, telegramId, move); if (applied.reason === 'waiting_for_opponent') { await sendTelegramMessage(tok, chatId, 'Move locked. Waiting for opponent.'); return; } if (applied.reason === 'move_already_locked') { await sendTelegramMessage(tok, chatId, 'Move already locked for this round. Waiting for the next round.'); return; } await sendTelegramMessage(tok, chatId, formatPetArenaRoundPrompt(applied.battle || battle), { reply_markup: (applied.battle?.status === 'active') ? buildPetArenaMoveReplyMarkup(battleId) : undefined }); return; }
  ...
  if (arg === 'app' || chatType === 'private') { const appPet = { ...pet, telegram_id: 'app', pet_name: 'App Moonpet', pet_xp: Math.max(0, Number(pet.pet_xp || 0) + 80), energy: 82, health: 88, happiness: 80, cleanliness: 80 }; const battle = await createPetArenaBattle(db, chatId, pet, appPet, 'app'); await sendTelegramMessage(tok, chatId, formatPetArenaRoundPrompt(battle), { reply_markup: buildPetArenaMoveReplyMarkup(battle.battle_id) }); return; }
