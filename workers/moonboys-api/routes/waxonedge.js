...
const PRICE_SNAPSHOT_SOURCE = 'price_snapshots';
...
const CANDLE_BACKFILL_PLAN = 'Legacy OHLC candle generation is disabled. WaxOnEdge stores lightweight token price snapshots instead.';
const TRADE_INDEX_PLAN = 'Alcor market match trade-row indexing is retained only for trade/volume diagnostics; no OHLC candles are built.';
...
function pairTokenContractBlocklistSql(alias = '') {
  ...
  return `COALESCE(LOWER(${prefix}token_a_contract), '') NOT IN (${placeholders}) AND COALESCE(LOWER(${prefix}token_b_contract), '') NOT IN (${placeholders})`;
}
...
function pairPublicLiquiditySanitySql(alias = '') {
  const prefix = alias ? `${alias}.` : '';
  return `CAST(COALESCE(${prefix}liquidity_wax, '0') AS NUMERIC) >= 0 AND CAST(COALESCE(${prefix}liquidity_wax, '0') AS NUMERIC) <= ? AND CAST(COALESCE(${prefix}liquidity_usd, '0') AS NUMERIC) >= 0 AND CAST(COALESCE(${prefix}liquidity_usd, '0') AS NUMERIC) <= ?`;
}

function pairPublicLiquiditySanityParams() {
  return [MAX_REASONABLE_PAIR_TVL_WAX, MAX_REASONABLE_PAIR_TVL_USD];
}

function publicPairFeedSql(alias = '') {
  return `${pairTokenContractBlocklistSql(alias)} AND ${pairPublicLiquiditySanitySql(alias)}`;
}

function publicPairFeedParams() {
  return pairTokenContractBlocklistParams().concat(pairPublicLiquiditySanityParams());
}

function publicPairFeedPolicy() {
  return {
...
const WAXONEDGE_RETENTION = {
  tradeRowsDays: 32,
  rawJsonDays: 2,
  intradayCandleDays: 32,
  dailyCandleDays: 90,
  holderSnapshotsToKeep: 3,
  syncRunDays: 14,
  batchLimit: 500,
  maxSnapshotRowsPerCleanup: 20,
  snapshotExampleLimit: 5,
};

function retentionIso(nowMs, days) {
  return new Date(nowMs - days * 24 * 60 * 60 * 1000).toISOString();
}

function waxonedgeRetentionCutoffs(now = new Date()) {
  const nowMs = now instanceof Date ? now.getTime() : Date.parse(now);
  const safeNow = Number.isFinite(nowMs) ? nowMs : Date.now();
  return {
    trades: retentionIso(safeNow, WAXONEDGE_RETENTION.tradeRowsDays),
    raw_json: retentionIso(safeNow, WAXONEDGE_RETENTION.rawJsonDays),
    intraday_candles: retentionIso(safeNow, WAXONEDGE_RETENTION.intradayCandleDays),
    daily_candles: retentionIso(safeNow, WAXONEDGE_RETENTION.dailyCandleDays),
    sync_runs: retentionIso(safeNow, WAXONEDGE_RETENTION.syncRunDays),
  };
}

async function runLimitedMutation(db, sql, params = []) {
  const result = await db.prepare(sql).bind(...params).run();
  return Number(result?.meta?.changes ?? result?.changes ?? 0) || 0;
}

function capDiagnosticArray(value, limit = WAXONEDGE_RETENTION.snapshotExampleLimit) {
  return Array.isArray(value) && value.length > limit ? value.slice(0, limit) : value;
}

function compactNormalizationDiagnostics(diagnostics, limit = WAXONEDGE_RETENTION.snapshotExampleLimit) {
  if (!diagnostics || typeof diagnostics !== 'object') return diagnostics;
  let changed = false;
  const compact = Array.isArray(diagnostics) ? diagnostics.slice() : { ...diagnostics };
  for (const key of Object.keys(compact)) {
    const value = compact[key];
    if (Array.isArray(value)) {
      const capped = capDiagnosticArray(value, limit);
      if (capped !== value) changed = true;
      compact[key] = capped;
    } else if (value && typeof value === 'object' && !Array.isArray(value)) {
      const nested = compactNormalizationDiagnostics(value, limit);
      if (nested !== value) changed = true;
      compact[key] = nested;
    }
  }
  return changed ? compact : diagnostics;
}

function compactWaxonedgeSnapshotPayload(payload, limit = WAXONEDGE_RETENTION.snapshotExampleLimit) {
  if (!payload || typeof payload !== 'object') return { payload, changed: false };
  let changed = false;
  const compact = Array.isArray(payload) ? payload.slice() : { ...payload };
  const capKeys = [
    'raw_waxcash_examples',
    'raw_graffitiking_non_waxcash_examples',
    'rejected_examples',
    'first_20_og_bucket_keys',
    'lookup_keys_attempted',
  ];
  for (const key of capKeys) {
    if (Array.isArray(compact[key]) && compact[key].length > limit) {
      compact[key] = compact[key].slice(0, limit);
      changed = true;
    }
  }
  if (compact.normalization_diagnostics) {
    const next = compactNormalizationDiagnostics(compact.normalization_diagnostics, limit);
    if (next !== compact.normalization_diagnostics) changed = true;
    compact.normalization_diagnostics = next;
  }
  if (compact.source_snapshot_diagnostics && typeof compact.source_snapshot_diagnostics === 'object') {
    const nextDiagnostics = { ...compact.source_snapshot_diagnostics };
    for (const [source, value] of Object.entries(nextDiagnostics)) {
      const nextValue = compactNormalizationDiagnostics(value, limit);
      if (nextValue !== value) changed = true;
      nextDiagnostics[source] = nextValue;
    }
    compact.source_snapshot_diagnostics = nextDiagnostics;
  }
  return { payload: changed ? compact : payload, changed };
}

async function compactWaxonedgeSnapshots(db, limit = WAXONEDGE_RETENTION.maxSnapshotRowsPerCleanup) {
  const rows = await db.prepare(
    `SELECT source, payload_json FROM waxonedge_snapshots ORDER BY fetched_at DESC LIMIT ?`
  ).bind(limit).all().catch(() => ({ results: [] }));
  let compacted = 0;
  for (const row of sourceRows(rows?.results)) {
    if (!row?.source || !row?.payload_json) continue;
    let parsed = null;
    try {
    ...
    const compactedPayload = compactWaxonedgeSnapshotPayload(parsed);
    if (!compactedPayload.changed) continue;
    await db.prepare(
      `UPDATE waxonedge_snapshots SET payload_json = ? WHERE source = ?`
    ).bind(JSON.stringify(compactedPayload.payload), row.source).run();
    compacted += 1;
  }
  return compacted;
}

export async function runWaxOnEdgeRetentionCleanup(env, options = {}) {
  if (!env?.DB) return { ok: false, error: 'DB binding is not configured', no_fake_value: true };
  const batchLimit = clampInteger(options.batchLimit, WAXONEDGE_RETENTION.batchLimit, 1, 1000);
  const cutoffs = waxonedgeRetentionCutoffs(options.now || new Date());
  const db = env.DB;
  const rawJsonCompacted = await runLimitedMutation(db,
    `UPDATE waxonedge_trades
     SET raw_json = NULL
     WHERE rowid IN (
       SELECT rowid FROM waxonedge_trades
       WHERE raw_json IS NOT NULL AND traded_at < ?
       ORDER BY traded_at ASC
       LIMIT ?
     )`,
    [cutoffs.raw_json, batchLimit]);
  const tradesDeleted = await runLimitedMutation(db,
    `DELETE FROM waxonedge_trades
     WHERE rowid IN (
       SELECT rowid FROM waxonedge_trades
       WHERE traded_at < ?
       ORDER BY traded_at ASC
    ...
    [cutoffs.trades, batchLimit]);
  const intradayCandlesDeleted = await runLimitedMutation(db,
    `DELETE FROM waxonedge_chart_candles
     WHERE rowid IN (
       SELECT rowid FROM waxonedge_chart_candles
       WHERE interval IN ('1m','5m','15m','30m','1h') AND bucket_time < ?
       ORDER BY bucket_time ASC
       LIMIT ?
     )`,
    [cutoffs.intraday_candles, batchLimit]);
  const dailyCandlesDeleted = await runLimitedMutation(db,
    `DELETE FROM waxonedge_chart_candles
     WHERE rowid IN (
       SELECT rowid FROM waxonedge_chart_candles
       WHERE interval IN ('1D','1d','D') AND bucket_time < ?
       ORDER BY bucket_time ASC
       LIMIT ?
     )`,
    [cutoffs.daily_candles, batchLimit]);
  const holdersDeleted = await runLimitedMutation(db,
    `DELETE FROM waxonedge_holders
     WHERE rowid IN (
       SELECT h.rowid
       FROM waxonedge_holders h
       WHERE h.snapshot_at NOT IN (
         SELECT snapshot_at
         FROM waxonedge_holders keep
         WHERE keep.contract = h.contract AND keep.symbol = h.symbol
    ...
    [WAXONEDGE_RETENTION.holderSnapshotsToKeep, batchLimit]);
  const syncRunsDeleted = await runLimitedMutation(db,
    `DELETE FROM waxonedge_sync_runs
     WHERE id IN (
       SELECT id FROM waxonedge_sync_runs
       WHERE started_at < ?
       ORDER BY started_at ASC
       LIMIT ?
     )`,
    [cutoffs.sync_runs, batchLimit]);
  const snapshotsCompacted = await compactWaxonedgeSnapshots(db);
  return {
    ok: true,
    retention_policy: {
      trade_rows_days: WAXONEDGE_RETENTION.tradeRowsDays,
      raw_json_days: WAXONEDGE_RETENTION.rawJsonDays,
      intraday_candle_days: WAXONEDGE_RETENTION.intradayCandleDays,
      daily_candle_days: WAXONEDGE_RETENTION.dailyCandleDays,
      holder_snapshots_to_keep: WAXONEDGE_RETENTION.holderSnapshotsToKeep,
      sync_run_days: WAXONEDGE_RETENTION.syncRunDays,
      batch_limit: batchLimit,
    },
    cutoffs,
    trades_deleted: tradesDeleted,
    raw_json_compacted: rawJsonCompacted,
    candles_deleted: intradayCandlesDeleted + dailyCandlesDeleted,
    intraday_candles_deleted: intradayCandlesDeleted,
    daily_candles_deleted: dailyCandlesDeleted,
    holders_deleted: holdersDeleted,
    sync_runs_deleted: syncRunsDeleted,
...
function priceSnapshotKey(row) {
  return [
    normalizeContract(row?.contract),
    normalizeSymbol(row?.symbol),
    aggregateSourceKey(row?.source || ''),
    safeString(row?.pair_id || row?.pairId || ''),
  ].join('::');
}

function priceSnapshotChanged(previous, next) {
  if (!previous) return true;
  return ['price_wax', 'price_usd', 'liquidity_wax', 'volume_24h_wax', 'source', 'pair_id']
    .some((field) => String(previous?.[field] ?? '') !== String(next?.[field] ?? ''));
}

async function latestPriceSnapshotIndex(db) {
  const rows = await db.prepare(
    `SELECT contract, symbol, source, pair_id, price_wax, price_usd, liquidity_wax, volume_24h_wax
     FROM waxonedge_price_snapshots
     WHERE rowid IN (
       SELECT MAX(rowid)
       FROM waxonedge_price_snapshots
       GROUP BY contract, symbol, source, pair_id
     )`
  ).all().catch(() => ({ results: [] }));
  const index = new Map();
  for (const row of rows.results || []) {
    index.set(priceSnapshotKey(row), row);
  }
  return index;
}

async function recordPriceSnapshots(db, candidates, timestamp = nowIso()) {
  const latest = await latestPriceSnapshotIndex(db);
  const statements = [];
  const seen = new Set();
  for (const candidate of candidates || []) {
    const row = {
      ...
      source: aggregateSourceKey(candidate?.source || ''),
      pair_id: safeString(candidate?.pair_id || ''),
      price_wax: safeDecimal(asNumber(candidate?.price_wax)),
      price_usd: safeDecimal(asNumber(candidate?.price_usd)),
      liquidity_wax: safeDecimal(asNumber(candidate?.liquidity_wax)),
      volume_24h_wax: safeDecimal(asNumber(candidate?.volume_24h_wax)),
    };
    const key = priceSnapshotKey(row);
    if (!row.contract || !row.symbol || seen.has(key)) continue;
    if (row.price_wax == null && row.price_usd == null && row.liquidity_wax == null && row.volume_24h_wax == null) continue;
    seen.add(key);
    if (!priceSnapshotChanged(latest.get(key), row)) continue;
    statements.push(db.prepare(
      `INSERT OR IGNORE INTO waxonedge_price_snapshots
       (timestamp, contract, symbol, source, pair_id, price_wax, price_usd, liquidity_wax, volume_24h_wax)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      row.timestamp,
      row.contract,
  ...
  return { ok: true, attempted: seen.size, written: statements.length, source: PRICE_SNAPSHOT_SOURCE };
}
...
async function aggregateTokenAnalytics(env) {
  ...
  for (const agg of aggregates.values()) {
    ...
    priceSnapshotCandidates.push({
      contract: agg.contract,
      symbol: agg.symbol,
      source: detailStats.selected_pair_source,
      pair_id: detailStats.selected_pair_id,
      price_wax: detailStats.selected_price_wax,
      price_usd: detailStats.selected_price_usd,
      liquidity_wax: detailStats.liquidity_wax,
      volume_24h_wax: detailStats.volume_24h_wax,
    });
  ...
  const priceSnapshots = await recordPriceSnapshots(env.DB, priceSnapshotCandidates, startedAt).catch((error) => ({
    ok: false,
    written: 0,
    error: error?.message || String(error),
  ...
  return { ok: aggregateStatus !== 'failed', tokens: aggregates.size, status: aggregateStatus, runStatus, price_snapshots: priceSnapshots };
}
...
async function listTokenPriceHistory(db, contract, symbol, query = {}) {
  const limit = clampInteger(query.limit || query.countBack, 120, 1, 1000);
  const rows = await db.prepare(
    `SELECT timestamp, contract, symbol, source, pair_id, price_wax, price_usd, liquidity_wax, volume_24h_wax
     FROM waxonedge_price_snapshots
     WHERE contract = ? AND symbol = ?
     ORDER BY timestamp DESC
     LIMIT ?`
  ).bind(contract, symbol, limit).all().catch(() => ({ results: [] }));
  const pricePoints = (rows.results || []).reverse().map((row) => ({
    ...
    price_wax: row.price_wax,
    price_usd: row.price_usd,
    liquidity_wax: row.liquidity_wax,
    volume_24h_wax: row.volume_24h_wax,
  }));
  const current = await getToken(db, contract, symbol).catch(() => ({ token: null, stats: null }));
  const currentPriceWax = current.stats?.selected_price_wax ?? current.token?.price_wax ?? null;
  const currentPriceUsd = current.stats?.selected_price_usd ?? current.token?.price_usd ?? null;
  const currentState = current.token ? {
    timestamp: current.stats?.updated_at || current.token?.updated_at || null,
    contract,
    symbol,
    source: current.stats?.selected_pair_source || null,
    pair_id: current.stats?.selected_pair_id || null,
    price_wax: currentPriceWax,
    price_usd: currentPriceUsd,
    liquidity_wax: current.stats?.liquidity_wax || null,
    volume_24h_wax: current.stats?.volume_24h_wax || current.stats?.volume_24h || null,
  } : null;
  const points = pricePoints.length ? pricePoints : (currentState?.timestamp && (currentPriceWax != null || currentPriceUsd != null) ? [currentState] : []);
  return {
    chart_source: points[points.length - 1] ? {
      source: points[points.length - 1].source,
      pair_id: points[points.length - 1].pair_id,
      storage_model: 'waxonedge_price_snapshots',
    } : null,
    price_points: points,
    current_state: currentState,
    storage_model: 'waxonedge_price_snapshots',
    candles: [],
    unavailable: points.length ? null : 'price_snapshots_not_available',
    no_ohlc_candles: true,
  };
}

function priceChange24hFromPricePoints(points = []) {
  const samples = (points || [])
    .map((point) => ({
      millis: Date.parse(point.timestamp || point.time || ''),
      price: asNumber(point.price_wax ?? point.price_usd),
    }))
    .filter((point) => Number.isFinite(point.millis) && point.price != null && point.price > 0)
    .sort((a, b) => a.millis - b.millis);
  if (samples.length < 2) return null;
  const latest = samples[samples.length - 1];
  const cutoff = latest.millis - (24 * 60 * 60 * 1000);
  let prior = null;
  for (const sample of samples) {
    if (sample.millis <= cutoff) prior = sample;
  }
  prior = prior || samples[0];
  if (!prior || prior.price <= 0 || prior.millis === latest.millis) return null;
...
function ogDirectWaxTokenPrice(contract, symbol, directWaxPairs = [], priceIndex = new Map()) {
  ...
  for (const pair of candidates) {
    ...
    if (waxReserve == null || waxReserve <= 0 || tokenReserve == null || tokenReserve <= 0) continue;
  ...
  if (!selected) {
    return {
      ...
      token_reserve: null,
      updated_at: null,
      reason_codes: candidates.length ? ['no_direct_wax_pool_with_usable_reserves'] : ['no_direct_wax_pool'],
    };
  }
...
async function loadPairRowsForToken(db, contract, symbol) {
  ...
  ).bind(contract, symbol, contract, symbol, ...publicPairFeedParams()).all().catch(() => ({ results: [] }));
  return rows.results || [];
}
  ...
  const rows = await db.prepare(
    `SELECT source, pair_id, og_laststats_pair_id, token_a_contract, token_a_symbol, token_b_contract, token_b_symbol,
     ...
     WHERE ((token_a_contract = ? AND token_a_symbol = ?)
        OR (token_b_contract = ? AND token_b_symbol = ?))
       AND ${publicPairFeedSql()}`
  ).bind(WAXCASH_CONTRACT, WAXCASH_SYMBOL, WAXCASH_CONTRACT, WAXCASH_SYMBOL, ...publicPairFeedParams()).all().catch(() => ({ results: [] }));
  return rows.results || [];
}
  ...
  for (let i = 0; i < targets.length; i += 50) {
    ...
    const result = await db.prepare(
      `SELECT source, pair_id, token_a_contract, token_a_symbol, token_b_contract, token_b_symbol,
       ...
       WHERE (${where})
         AND ${publicPairFeedSql()}`
    ).bind(...params, ...publicPairFeedParams()).all().catch(() => ({ results: [] }));
    rows.push(...(result.results || []));
  }
...
function waxcashLiteMemberFromPair(row) {
  if (!row) return null;
  if (isWaxcashToken(row.token_a_contract, row.token_a_symbol)) {
    return {
      contract: normalizeContract(row.token_b_contract),
      symbol: normalizeSymbol(row.token_b_symbol),
      icon_url: row.token_b_icon || row.token_b_logo || null,
    };
  }
  if (isWaxcashToken(row.token_b_contract, row.token_b_symbol)) {
    return {
      contract: normalizeContract(row.token_a_contract),
      symbol: normalizeSymbol(row.token_a_symbol),
      icon_url: row.token_a_icon || row.token_a_logo || null,
    };
  }
  return null;
}

function waxcashLitePairLabel(row) {
  const a = normalizeSymbol(row?.token_a_symbol);
  const b = normalizeSymbol(row?.token_b_symbol);
  const label = a && b ? `${a}/${b}` : 'WAXCASH pair';
  return row?.pair_id ? `${label} #${row.pair_id}` : label;
}

function waxcashLitePairBubbleCandidate(row, generatedAt) {
  const member = waxcashLiteMemberFromPair(row);
  if (!member?.contract || !member?.symbol) return null;
  const source = aggregateSourceKey(row.source);
  const candidateSelectedPriceWax = row.price_wax ?? row.selected_price_wax ?? row.price ?? row.pair_price ?? null;
  const candidateSelectedPriceUsd = row.price_usd ?? row.selected_price_usd ?? null;
  return {
    ...
    pair_id: row.pair_id || null,
    source,
    selected_price_wax: candidateSelectedPriceWax,
    selected_price_usd: candidateSelectedPriceUsd,
    change_24h: row.change_24h ?? null,
    volume_24h_wax: row.volume_24h_wax ?? null,
    volume_24h_usd: row.volume_24h_usd ?? null,
    volume_7d_wax: row.volume_7d_wax ?? null,
    volume_7d_usd: row.volume_7d_usd ?? null,
    volume_30d_wax: row.volume_30d_wax ?? null,
    volume_30d_usd: row.volume_30d_usd ?? null,
    graph_liquidity_wax: row.liquidity_wax ?? null,
    graph_liquidity_usd: row.liquidity_usd ?? null,
    liquidity_wax: row.liquidity_wax ?? null,
    liquidity_usd: row.liquidity_usd ?? null,
    tvl_wax: row.liquidity_wax ?? null,
    tvl_usd: row.liquidity_usd ?? null,
    market_cap_wax: null,
    market_cap_usd: null,
    ...
    selected_pair_source: source || null,
    selected_pair_id: row.pair_id || null,
    waxcash_pair_source: row.source || source || null,
    waxcash_pair_id: row.pair_id || null,
    waxcash_pair_label: waxcashLitePairLabel(row),
    waxcash_pair_valuation_basis: row.valuation_basis || null,
    valuation_basis: row.valuation_basis || null,
    metric_status: {
      selected_price: candidateSelectedPriceWax != null || candidateSelectedPriceUsd != null ? { live: true, source: 'waxcash_pair_row' } : { live: false, reason: 'pair_price_unavailable' },
      liquidity: row.liquidity_wax != null || row.liquidity_usd != null ? { live: true, source: 'waxcash_pair_row' } : { live: false, reason: 'pair_liquidity_unavailable' },
      tvl: row.liquidity_wax != null || row.liquidity_usd != null ? { live: true, source: 'waxcash_pair_row' } : { live: false, reason: 'pair_tvl_unavailable' },
      market_cap: { live: false, reason: 'phase_one_pair_member_market_cap_unavailable' },
    },
...
async function loadWaxcashLiteRootSummary(db) {
  try {
    const row = await db.prepare(
      `SELECT t.contract, t.symbol, t.icon_url, t.updated_at AS token_updated_at,
              s.holder_count, s.circulating_supply, s.volume_24h, s.volume_24h_wax, s.volume_24h_usd,
              s.volume_7d, s.volume_30d, s.market_cap_wax, s.market_cap_usd, s.fdv_wax, s.fdv_usd,
              s.liquidity_wax, s.liquidity_usd, s.tvl_wax, s.tvl_usd, s.selected_price_wax,
              s.selected_price_usd, s.change_24h, s.selected_pair_source, s.selected_pair_id,
              s.source_count, s.indexed_pair_count, s.source_keys, s.updated_at
       FROM waxonedge_tokens t
       LEFT JOIN waxonedge_token_stats s ON s.contract = t.contract AND s.symbol = t.symbol
       WHERE t.contract = ? AND t.symbol = ?
       LIMIT 1`
    ).bind(WAXCASH_CONTRACT, WAXCASH_SYMBOL).first();
    return {
      ok: true,
      row: row || {
        contract: WAXCASH_CONTRACT,
...
async function loadWaxcashLitePairRows(db) {
  try {
    const rows = await db.prepare(
      `SELECT p.source, p.pair_id, p.token_a_contract, p.token_a_symbol, p.token_b_contract, p.token_b_symbol,
              p.price, p.change_24h, p.volume_24h, p.volume_24h_wax, p.volume_24h_usd,
              p.volume_7d, p.volume_7d_wax, p.volume_7d_usd, p.volume_30d, p.volume_30d_wax, p.volume_30d_usd,
              p.liquidity_wax, p.liquidity_usd, p.reserve_a, p.reserve_b, p.fee_bps, p.updated_at,
              ta.icon_url AS token_a_icon, tb.icon_url AS token_b_icon
       FROM waxonedge_pairs p
       LEFT JOIN waxonedge_tokens ta ON ta.contract = p.token_a_contract AND ta.symbol = p.token_a_symbol
       LEFT JOIN waxonedge_tokens tb ON tb.contract = p.token_b_contract AND tb.symbol = p.token_b_symbol
       WHERE (p.token_a_contract = ? AND p.token_a_symbol = ?)
          OR (p.token_b_contract = ? AND p.token_b_symbol = ?)
       ORDER BY CAST(COALESCE(p.liquidity_wax, '0') AS NUMERIC) DESC,
                CAST(COALESCE(p.volume_24h_wax, p.volume_24h, '0') AS NUMERIC) DESC,
                p.source ASC,
                p.pair_id ASC`
...
function buildWaxcashBubblesLiteFromRows(rootStats = {}, pairRows = [], generatedAt = nowIso(), options = {}) {
  ...
  const pairQueryError = pairRowsAvailable ? null : (options.pairQueryError || options.liteQueryError || 'waxcash_lite_pair_rows_query_failed');
  const queryErrors = [rootQueryError, pairQueryError].filter(Boolean);
  ...
  const selectedUsd = asNumber(rootStats.selected_price_usd);
  const waxPriceUsd = selectedWax != null && selectedWax > 0 && selectedUsd != null ? selectedUsd / selectedWax : null;
  const sourceKeys = String(rootStats.source_keys || '').split(',').map(aggregateSourceKey).filter(Boolean);
  const root = {
    symbol: WAXCASH_SYMBOL,
    contract: WAXCASH_CONTRACT,
    icon_url: rootStats.icon_url || null,
    visible_in_waxcash_bubbles: true,
    selected_price_wax: rootStats.selected_price_wax ?? null,
    selected_price_usd: rootStats.selected_price_usd ?? null,
    change_24h: rootStats.change_24h ?? null,
    volume_24h_wax: rootStats.volume_24h_wax ?? rootStats.volume_24h ?? null,
    volume_24h_usd: rootStats.volume_24h_usd ?? null,
    volume_7d_wax: rootStats.volume_7d_wax ?? rootStats.volume_7d ?? null,
    volume_7d_usd: rootStats.volume_7d_usd ?? null,
    volume_30d_wax: rootStats.volume_30d_wax ?? rootStats.volume_30d ?? null,
    volume_30d_usd: rootStats.volume_30d_usd ?? null,
    graph_liquidity_wax: rootStats.liquidity_wax ?? null,
    graph_liquidity_usd: rootStats.liquidity_usd ?? null,
    liquidity_wax: rootStats.liquidity_wax ?? null,
    liquidity_usd: rootStats.liquidity_usd ?? null,
    tvl_wax: rootStats.tvl_wax ?? rootStats.liquidity_wax ?? null,
    tvl_usd: rootStats.tvl_usd ?? rootStats.liquidity_usd ?? null,
    market_cap_wax: rootStats.market_cap_wax ?? null,
    market_cap_usd: rootStats.market_cap_usd ?? null,
    holder_count: rootStats.holder_count ?? null,
    holder_count_live: rootStats.holder_count != null,
    circulating_supply: rootStats.circulating_supply ?? null,
    source_keys: sourceKeys.length ? sourceKeys : Object.keys(sourceCounts),
    source_count: rootStats.source_count ?? (sourceKeys.length || Object.keys(sourceCounts).length),
    indexed_pair_count: pairRowsAvailable ? (rootStats.indexed_pair_count ?? pairRows.length) : null,
    selected_price_source: rootStats.selected_pair_source || null,
    selected_pair_source: rootStats.selected_pair_source || null,
    selected_pair_id: rootStats.selected_pair_id || null,
    valuation_basis: rootStats.market_cap_wax != null || rootStats.market_cap_usd != null ? 'indexed_token_stats' : null,
    metric_status: {
      selected_price: rootStats.selected_price_wax != null || rootStats.selected_price_usd != null ? { live: true, source: 'indexed_token_stats' } : { live: false, reason: 'selected_price_unavailable' },
      holder_count: rootStats.holder_count != null ? { live: true, source: 'indexed_token_stats' } : { live: false, reason: 'holder_count_unavailable' },
      liquidity: rootStats.liquidity_wax != null || rootStats.liquidity_usd != null ? { live: true, source: 'indexed_token_stats' } : { live: false, reason: 'liquidity_unavailable' },
      tvl: rootStats.tvl_wax != null || rootStats.tvl_usd != null ? { live: true, source: 'indexed_token_stats' } : { live: false, reason: 'tvl_unavailable' },
      market_cap: rootStats.market_cap_wax != null || rootStats.market_cap_usd != null ? { live: true, source: 'indexed_token_stats' } : { live: false, reason: 'market_cap_unavailable' },
    },
  ...
  for (const row of pairRows) {
    const candidate = waxcashLitePairBubbleCandidate(row, generatedAt);
    if (!candidate) continue;
    const key = tokenKey(candidate.contract, candidate.symbol);
    byKey.set(key, waxcashLiteMergeMemberToken(byKey.get(key), candidate));
  }
  const tokens = [root, ...Array.from(byKey.values())];
  const pairs = pairRows.map((row) => ({
    token_a_contract: row.token_a_contract || null,
    token_a_symbol: row.token_a_symbol || null,
    token_a_icon: row.token_a_icon || row.token_a_logo || null,
    token_b_contract: row.token_b_contract || null,
    token_b_symbol: row.token_b_symbol || null,
    token_b_icon: row.token_b_icon || row.token_b_logo || null,
    pair_label: waxcashLitePairLabel(row),
    pair_id: row.pair_id || null,
    source: row.source || null,
    price_wax: row.price_wax ?? row.selected_price_wax ?? row.price ?? row.pair_price ?? null,
    price_usd: row.price_usd ?? row.selected_price_usd ?? null,
    change_24h: row.change_24h ?? null,
    volume_24h_wax: row.volume_24h_wax ?? null,
    volume_24h_usd: row.volume_24h_usd ?? null,
    volume_7d_wax: row.volume_7d_wax ?? null,
  ...
  return {
    ...
    summary: {
      ...
      wax_price_source: waxPriceUsd == null ? null : 'derived_from_waxcash_selected_price',
      metric_capabilities: metricCapabilitiesFromTokens(tokens),
      source_feed: `${WAXONEDGE_API_PREFIX}/waxcash-bubbles-lite`,
    },
    metric_capabilities: metricCapabilitiesFromTokens(tokens),
    payload_policy: {
      slim: true,
      data_source: 'dedicated_slim_d1_queries',
      excludes_full_waxcash_analytics_build: true,
      excludes_source_stability_debug: true,
      excludes_source_index_state_rows: true,
      excludes_source_snapshot_diagnostics: true,
      excludes_normalization_diagnostics: true,
      excludes_chart_candles: true,
      no_fake_value: true,
    },
    root_summary_available: rootSummaryAvailable,
    pair_rows_available: pairRowsAvailable,
...
function waxcashLiteBetterToken(left, right) {
  ...
  const leftValue = asNumber(left.liquidity_wax) ?? asNumber(left.liquidity_usd) ?? asNumber(left.volume_24h_wax) ?? 0;
  const rightValue = asNumber(right.liquidity_wax) ?? asNumber(right.liquidity_usd) ?? asNumber(right.volume_24h_wax) ?? 0;
  return rightValue > leftValue ? right : left;
}

function waxcashLiteMergeMemberToken(existing, candidate) {
  if (!existing) return candidate;
  const sourceKeys = Array.from(new Set([...(existing.source_keys || []), ...(candidate.source_keys || [])]))
    .filter(Boolean)
    .sort();
  const indexedPairCount = (asNumber(existing.indexed_pair_count) || 0) + 1;
  const winner = waxcashLiteBetterToken(existing, candidate) === candidate ? { ...candidate } : { ...existing };
  winner.source_keys = sourceKeys;
  winner.source_count = sourceKeys.length;
  winner.indexed_pair_count = indexedPairCount;
  return winner;
}

function waxcashLiteMembershipMap(pairRows = []) {
  const map = new Map();
  for (const row of pairRows || []) {
    const member = waxcashLiteMemberFromPair(row);
    if (!member?.contract || !member?.symbol) continue;
    const key = tokenKey(member.contract, member.symbol);
    const source = aggregateSourceKey(row.source);
    const next = {
      contract: member.contract,
      symbol: member.symbol,
      ...
      waxcash_pair_id: row.pair_id || null,
      waxcash_pair_source: row.source || source || null,
      waxcash_pair_source_key: source || null,
      waxcash_pair_valuation_basis: row.valuation_basis || null,
      waxcash_pair_liquidity_wax: row.liquidity_wax ?? null,
      waxcash_pair_liquidity_usd: row.liquidity_usd ?? null,
      waxcash_pair_updated_at: row.updated_at || null,
    };
    const existing = map.get(key);
    const existingScore = asNumber(existing?.waxcash_pair_liquidity_wax) ?? asNumber(existing?.waxcash_pair_liquidity_usd) ?? 0;
    const nextScore = asNumber(next.waxcash_pair_liquidity_wax) ?? asNumber(next.waxcash_pair_liquidity_usd) ?? 0;
    if (!existing || nextScore > existingScore) map.set(key, next);
  }
  return map;
}

function waxcashLiteValuationBasis(row, membership = null) {
  const route = String(row?.selected_price_route || row?.selected_price_proof?.route_type || '').toLowerCase();
  const hops = Array.isArray(row?.selected_price_proof?.route_hops) ? row.selected_price_proof.route_hops : [];
  const membershipSource = aggregateSourceKey(membership?.waxcash_pair_source || membership?.waxcash_pair_source_key);
  ...
  return row?.valuation_basis || row?.liquidity_basis || null;
}

function tokenKeyFromRoutePart(value) {
  const text = safeString(value);
  if (!text) return '';
  if (text.includes('::')) {
    const [contract, symbol] = text.split('::');
    return tokenKey(contract, symbol);
  }
  return text.toLowerCase();
}

function waxcashLiteFullTokenBubble(row, membership, generatedAt) {
  const contract = normalizeContract(row?.contract);
  const symbol = normalizeSymbol(row?.symbol);
  if (!contract || !symbol || !membership || isWaxcashToken(contract, symbol)) return null;
  const proof = tokenMetricProof(row);
  const priceLive = proof.metric_status?.selected_price?.live === true;
  const liquidityLive = proof.metric_status?.liquidity?.live === true;
  const tvlLive = proof.metric_status?.tvl?.live === true;
  const marketCapLive = proof.metric_status?.market_cap?.live === true;
  ...
  return {
    symbol,
    contract,
    icon_url: row.icon_url || membership.icon_url || null,
    visible_in_waxcash_bubbles: true,
    ...
    selected_price_route: row.selected_price_route || row.selected_price_proof?.route_type || null,
    valuation_basis: valuationBasis,
    liquidity_wax: liquidityLive ? row.liquidity_wax ?? null : null,
    liquidity_usd: liquidityLive ? row.liquidity_usd ?? null : null,
    liquidity_confidence: proof.liquidity_confidence,
    graph_liquidity_wax: liquidityLive ? row.graph_liquidity_wax ?? row.liquidity_wax ?? null : null,
    graph_liquidity_usd: liquidityLive ? row.graph_liquidity_usd ?? row.liquidity_usd ?? null : null,
    tvl_wax: tvlLive ? row.tvl_wax ?? null : null,
    tvl_usd: tvlLive ? row.tvl_usd ?? null : null,
    tvl_confidence: proof.tvl_confidence,
    market_cap_wax: marketCapLive ? row.market_cap_wax ?? null : null,
    market_cap_usd: marketCapLive ? row.market_cap_usd ?? null : null,
    market_cap_confidence: marketCapLive ? 'good' : 'unavailable',
    volume_24h_wax: row.volume_24h_wax ?? row.volume_24h ?? null,
    volume_24h_usd: row.volume_24h_usd ?? null,
    volume_7d_wax: row.volume_7d_wax ?? row.volume_7d ?? null,
    volume_7d_usd: row.volume_7d_usd ?? null,
    volume_30d_wax: row.volume_30d_wax ?? row.volume_30d ?? null,
    volume_30d_usd: row.volume_30d_usd ?? null,
    change_24h: row.change_24h ?? null,
    holder_count: row.holder_count ?? null,
    source_keys: sourceKeys,
    source_count: asNumber(row.source_count) ?? sourceKeys.length,
    indexed_pair_count: asNumber(row.indexed_pair_count ?? row.pair_count),
    strongest_pair: row.strongest_pair || null,
    strongest_pair_source: row.strongest_pair?.source || row.selected_pair_source || null,
    strongest_pair_id: row.strongest_pair?.pair_id || row.selected_pair_id || null,
    ...
    metric_status: proof.metric_status,
    metric_reason_codes: proof.metric_reason_codes,
    unavailable_reasons: row.unavailable_reasons || reasonMapForTokenMetrics(row),
    full_token_analytics_source: 'all_indexed_wax_dex_graph',
    membership_source: 'direct_waxcash_pair',
    updated_at: row.stats_updated_at || row.updated_at || membership.waxcash_pair_updated_at || generatedAt,
    no_fake_value: true,
  };
}
...
function waxcashLiteEnrichmentDiagnostics(tokens = [], membershipMap = new Map()) {
  ...
  return {
    ...
    tokens_unvalued_with_reason: unvalued.length,
    source_count_distribution: waxcashLiteDistribution(memberTokens.map((token) => token.source_count)),
    indexed_pair_count_distribution: waxcashLiteDistribution(memberTokens.map((token) => token.indexed_pair_count)),
    valuation_basis_counts: basisCounts,
    top_unvalued_examples: unvalued.slice(0, 8).map((token) => ({
      contract: token.contract,
      symbol: token.symbol,
...
function buildWaxcashBubblesLiteFromGraph(rootStats = {}, graph = {}, enrichedRows = [], generatedAt = nowIso(), options = {}) {
  ...
  const root = {
    ...
    selected_price_wax: rootStats.selected_price_wax ?? null,
    selected_price_usd: rootStats.selected_price_usd ?? null,
    selected_price_confidence: rootStats.selected_price_wax != null || rootStats.selected_price_usd != null ? 'good' : 'unavailable',
    change_24h: rootStats.change_24h ?? null,
    volume_24h_wax: rootStats.volume_24h_wax ?? rootStats.volume_24h ?? null,
    volume_24h_usd: rootStats.volume_24h_usd ?? null,
    volume_7d_wax: rootStats.volume_7d_wax ?? rootStats.volume_7d ?? null,
    ...
    liquidity_wax: rootStats.liquidity_wax ?? null,
    liquidity_usd: rootStats.liquidity_usd ?? null,
    liquidity_confidence: rootStats.liquidity_wax != null || rootStats.liquidity_usd != null ? 'good' : 'unavailable',
    tvl_wax: rootStats.tvl_wax ?? rootStats.liquidity_wax ?? null,
    tvl_usd: rootStats.tvl_usd ?? rootStats.liquidity_usd ?? null,
    tvl_confidence: rootStats.tvl_wax != null || rootStats.tvl_usd != null ? 'good' : 'unavailable',
    market_cap_wax: rootStats.market_cap_wax ?? null,
    market_cap_usd: rootStats.market_cap_usd ?? null,
    market_cap_confidence: rootStats.market_cap_wax != null || rootStats.market_cap_usd != null ? 'good' : 'unavailable',
    holder_count: rootStats.holder_count ?? null,
    holder_count_live: rootStats.holder_count != null,
    circulating_supply: rootStats.circulating_supply ?? null,
    source_keys: sourceKeys.length ? sourceKeys : Object.keys(sourceCounts),
    source_count: rootStats.source_count ?? (sourceKeys.length || Object.keys(sourceCounts).length),
    indexed_pair_count: rootStats.indexed_pair_count ?? pairRows.length,
    selected_price_source: rootStats.selected_pair_source || null,
    selected_pair_source: rootStats.selected_pair_source || null,
    selected_pair_id: rootStats.selected_pair_id || null,
    valuation_basis: rootStats.market_cap_wax != null || rootStats.market_cap_usd != null ? 'indexed_token_stats' : null,
  ...
  return {
    ...
    metric_capabilities: metricCapabilitiesFromTokens(tokens),
    enrichment_debug: waxcashLiteEnrichmentDiagnostics(tokens, membershipMap),
    payload_policy: {
      slim: true,
...
function buildWaxcashBubblesLiteFromAnalytics(analytics) {
  const generatedAt = nowIso();
  const stats = analytics.stats || {};
  const token = analytics.token || {};
  const pairRows = sourceRows(analytics.sections?.pair_table?.rows);
  const sourceCounts = countRowsByAggregateSource(pairRows);
  const root = {
    symbol: WAXCASH_SYMBOL,
    contract: WAXCASH_CONTRACT,
    ...
    visible_in_waxcash_bubbles: true,
    selected_price_wax: stats.selected_price_wax ?? null,
    selected_price_usd: stats.selected_price_usd ?? null,
    change_24h: stats.change_24h ?? null,
    volume_24h_wax: stats.volume_24h_wax ?? null,
    volume_24h_usd: stats.volume_24h_usd ?? null,
    volume_7d_wax: stats.volume_7d_wax ?? null,
    volume_7d_usd: stats.volume_7d_usd ?? null,
    volume_30d_wax: stats.volume_30d_wax ?? null,
    volume_30d_usd: stats.volume_30d_usd ?? null,
    graph_liquidity_wax: stats.cumulated_pair_liquidity_wax ?? stats.liquidity_wax ?? null,
    graph_liquidity_usd: stats.cumulated_pair_liquidity_usd ?? stats.liquidity_usd ?? null,
    liquidity_wax: stats.cumulated_pair_liquidity_wax ?? stats.liquidity_wax ?? null,
    liquidity_usd: stats.cumulated_pair_liquidity_usd ?? stats.liquidity_usd ?? null,
    tvl_wax: stats.tvl_wax ?? null,
    tvl_usd: stats.tvl_usd ?? null,
    market_cap_wax: stats.market_cap_wax ?? null,
    market_cap_usd: stats.market_cap_usd ?? null,
    holder_count: stats.holder_count ?? null,
    holder_count_live: stats.metric_status?.holder_count?.live === true,
    circulating_supply: stats.circulating_supply ?? null,
    total_supply: stats.total_supply ?? null,
    source_keys: String(stats.source_keys || '').split(',').map(aggregateSourceKey).filter(Boolean),
    source_count: stats.source_count ?? Object.keys(sourceCounts).length,
    indexed_pair_count: stats.indexed_pair_count ?? pairRows.length,
    selected_price_source: stats.selected_price_source || stats.selected_pair_source || null,
    selected_pair_source: stats.selected_pair_source || null,
    selected_pair_id: stats.selected_pair_id || null,
    valuation_basis: stats.market_cap_basis || stats.selected_price_basis || null,
    metric_status: stats.metric_status || null,
    updated_at: stats.updated_at || token.updated_at || analytics.updated_at || analytics.generated_at || generatedAt,
    no_fake_value: true,
  };
  const byKey = new Map();
  for (const row of pairRows) {
  ...
  const selectedWax = asNumber(stats.selected_price_wax);
  const selectedUsd = asNumber(stats.selected_price_usd);
  const waxPriceUsd = selectedWax != null && selectedWax > 0 && selectedUsd != null ? selectedUsd / selectedWax : null;
  const tokens = [root, ...Array.from(byKey.values())];
  const pairs = pairRows.map((row) => ({
    token_a_contract: row.token_a_contract || null,
    ...
    pair_id: row.pair_id || null,
    source: row.source || null,
    price_wax: row.price_wax ?? row.price ?? row.pair_price ?? null,
  ...
  return {
    generated_at: generatedAt,
    updated_at: analytics.updated_at || stats.updated_at || token.updated_at || generatedAt,
    source: 'waxcash_bubbles_lite',
    tokens,
    pairs,
    summary: {
...
async function buildWaxcashChartBundle(db, proof, headline, options = {}) {
  ...
  const priceHistory = await listTokenPriceHistory(db, WAXCASH_CONTRACT, WAXCASH_SYMBOL, {
    limit: options.limit || options.countBack || 120,
  });
  const chart = {
    ...priceHistory,
    candles: [],
    candle_normalization: null,
    legacy_candles_read_only: false,
    build_from_indexed_trades: null,
    unavailable: priceHistory.unavailable,
  };
...
async function selectedProofPriceChange24h(db, selectedWaxPool = null, selectedPriceWax = null) {
  ...
  void selectedWaxPool;
  void selectedPriceWax;
  const history = await listTokenPriceHistory(db, WAXCASH_CONTRACT, WAXCASH_SYMBOL, { limit: 240 })
    .catch(() => ({ price_points: [], unavailable: 'price_snapshots_not_available' }));
  const change = priceChange24hFromPricePoints(history.price_points || []);
  return {
    change_24h: safeDecimal(change),
    source: change != null ? 'waxonedge_price_snapshots' : null,
    basis: change != null ? 'latest price snapshot versus prior 24h price snapshot' : null,
    reason: change != null ? null : (history.unavailable || 'price_snapshots_not_available'),
  };
}

function tradingViewHistoryFromPricePoints(chart) {
  const points = (chart?.price_points || [])
    .map((point) => {
      const timeMs = Date.parse(point.timestamp || point.time || '');
      const price = asNumber(point.price_wax ?? point.price_usd);
      if (!Number.isFinite(timeMs) || price == null) return null;
      return {
        time: Math.floor(timeMs / 1000),
        value: price,
        close: price,
        volume: asNumber(point.volume_24h_wax) ?? 0,
      };
    })
    .filter(Boolean)
  ...
  return {
    ...
    t: points.map((point) => point.time),
    c: points.map((point) => point.close),
    v: points.map((point) => point.volume),
    price_points: points,
  };
}
...
async function buildWaxcashUdfChartFeed(db, query = {}) {
  ...
  const history = tradingViewHistoryFromPricePoints(chart);
  return {
    feed_format: 'tradingview_udf_price_history',
    symbol: 'WAXCASH/WAX',
    ticker: 'WAXCASH/WAX',
    description: 'WAXCASH priced in WAX from WaxOnEdge price snapshots',
    exchange: 'WaxOnEdge',
    type: 'crypto',
    ...
    storage_model: chart.storage_model || 'waxonedge_price_snapshots',
    no_ohlc_candles: true,
...
async function buildWaxcashAnalytics(db, env = null) {
  ...
  analytics.sections = {
    ...
    chart: {
      ...
      feed_format: 'tradingview_udf_price_history',
      storage_model: chart.storage_model || 'waxonedge_price_snapshots',
      price_points: chart.price_points || [],
      candles: [],
      no_ohlc_candles: true,
      build_from_indexed_trades: chart.build_from_indexed_trades || null,
      unavailable: chart.unavailable || null,
...
function diagnoseTokenAggregate(contract, symbol, metrics, pairRows, priceSnapshotCount, aggregateFresh) {
  const reasons = [];
  ...
  if (!priceSnapshotCount) {
    reasons.push('price snapshots missing');
  }
  if (!aggregateFresh) {
  ...
  return {
    ...
    facts: {
      ...
      price_snapshot_count: priceSnapshotCount,
...
async function countTokenPageSuppressedPairRows(db, contract, symbol) {
  const row = await db.prepare(
    `SELECT COUNT(*) AS count
     FROM waxonedge_pairs
     WHERE ((token_a_contract = ? AND token_a_symbol = ?)
...
async function getTokenDebug(db, contract, symbol) {
  ...
  const priceSnapshotCount = await countScalar(db,
    `SELECT COUNT(*) AS count
     FROM waxonedge_price_snapshots
     WHERE contract = ? AND symbol = ?`,
    [contract, symbol]);
  ...
  let nextAction = null;
  if (!priceSnapshotCount) {
    nextAction = 'waiting for price snapshot';
  ...
  return {
    ...
    price_history_endpoint: `${WAXONEDGE_API_PREFIX}/token/${encodeURIComponent(contract)}/${encodeURIComponent(symbol)}/chart`,
    legacy_candle_url_example: candleUrlExample(chartSrc, chartPairId),
    aggregate_totals: aggregateTotals,
    ...(waxcashOgProof || {}),
    diagnostics: diagnoseTokenAggregate(contract, symbol, detail.stats, pairRows, priceSnapshotCount, aggregateFresh),
    ...
    sync_diagnostics: {
      ...
      price_snapshot_count: priceSnapshotCount,
      has_price_snapshots: priceSnapshotCount > 0,
...
async function getIndexerHealth(db, env = {}) {
  ...
  const [
    ...
    countScalar(db, `SELECT COUNT(DISTINCT contract || '::' || symbol) AS count FROM waxonedge_price_snapshots`),
    countScalar(db,
      `SELECT COUNT(*) AS count
       FROM waxonedge_tokens t
  ...
  const priceSnapshotCount = await countScalar(db, `SELECT COUNT(*) AS count FROM waxonedge_price_snapshots`);
  const selectedChartExamplePair = await db.prepare(
  ...
  const deadReasons = {
    ...
    price_snapshots_missing: Math.max(0, totalTokens - tokensWithPriceSnapshots),
  ...
  return {
    ...
    totals: {
      ...
      tokens_with_price_snapshots: tokensWithPriceSnapshots,
      tokens_without_price_snapshots: Math.max(0, totalTokens - tokensWithPriceSnapshots),
    ...
    price_snapshot_history: {
      snapshot_count: priceSnapshotCount,
      tokens_with_no_price_snapshots: Math.max(0, totalTokens - tokensWithPriceSnapshots),
      tokens_with_pair_candidate_but_no_price_snapshots: Math.max(0, tokensWithPairs - tokensWithPriceSnapshots),
      storage_model: 'waxonedge_price_snapshots',
      no_ohlc_candles: true,
    },
    legacy_candles: {
      generation: 'disabled',
      backfill: 'disabled',
      legacy_read_endpoint: `${WAXONEDGE_API_PREFIX}/candles`,
      legacy_url_example: candleUrlExample(selectedChartExamplePair?.source || 'alcor', selectedChartExamplePair?.pair_id || '<selected_pair_id>'),
      no_new_candle_rows: true,
    },
    trade_indexing: {
      source: ALCOR_TRADE_INDEX_SOURCE,
    ...
    candle_backfill: {
      ...
      status: 'disabled',
      reason: 'legacy_ohlc_candle_generation_disabled',
      candles_written: 0,
      no_trade_scan: true,
      no_new_candle_rows: true,
...
function metricCapabilitiesFromTokens(tokens = []) {
  ...
  return {
    ...
    mcap: marketCapLive,
    volume_7d: has((token) => asNumber(token?.volume_7d_wax ?? token?.volume_7d_usd ?? token?.volume_7d) != null),
    volume_30d: has((token) => asNumber(token?.volume_30d_wax ?? token?.volume_30d_usd ?? token?.volume_30d) != null),
...
async function handleBootstrap(env, corsHeaders) {
  ...
  warnings.push('7d/30d volume, market cap, FDV, and price snapshot history stay unavailable until indexed from source data.');
  return ok({
    summary: {
    ...
    unavailable_metrics: {
      ...
      fdv: UNAVAILABLE,
      price_snapshots: SOURCE_NOT_INDEXED,
    },
...
export async function handleWaxOnEdgeRoute(request, env, corsHeaders = {}) {
  ...
  try {
    ...
    if (path === `${WAXONEDGE_API_PREFIX}/waxcash-bubbles-lite`) {
      ...
      if ((liteMode === 'membership' || liteMode === 'members') && lite?.ok === false) {
        return waxonedgeJson(envelope({
          ok: false,
          data: lite,
          warnings: [lite.lite_query_error || 'waxcash_membership_lite_unavailable'],
    ...
    if (path === `${WAXONEDGE_API_PREFIX}/retention-cleanup`) {
      const expectedSecret = String(env.ADMIN_SECRET || '').trim();
      const suppliedSecret = String(request.headers.get('x-admin-secret') || '').trim();
      if (!expectedSecret || suppliedSecret !== expectedSecret) {
        return unavailable('Forbidden', 403, corsHeaders);
      }
      const retention = await runWaxOnEdgeRetentionCleanup(env);
      return ok({
        cleanup_trigger: 'admin_diagnostic_route',
        retention,
        no_fake_value: true,
      }, ['Admin-only WaxOnEdge D1 retention cleanup run. Cloudflare D1 Time Travel is backup retention, not app-table cleanup.'], null, corsHeaders);
    }
    ...
    const tokenPageMatch = path.match(/^\/api\/waxonedge\/token-page\/([^/]+)\/([^/]+)$/);
    if (tokenPageMatch) {
    ...
    if (tokenMatch) {
      ...
      if (child === 'chart') {
        const chart = await listTokenPriceHistory(env.DB, contract, symbol, {
          limit: url.searchParams.get('limit') || url.searchParams.get('countBack'),
        });
        return ok(chart, chart.unavailable ? [chart.unavailable] : [], chart.price_points?.slice(-1)[0]?.timestamp || chart.current_state?.timestamp || null, corsHeaders);
      }
      if (child === 'debug') {
...
export const __waxonedgeTestHooks = {
  ...
  loadWaxcashOgPairRows,
  buildWaxcashBubblesLite,
  buildWaxcashBubblesLiteFromAnalytics,
  buildWaxcashBubblesLiteFromGraph,
  ...
  waxonedgeRetentionCutoffs,
  compactWaxonedgeSnapshotPayload,
...
async function runWaxOnEdgeRetentionMaintenance(env) {
  try {
    const retention = await runWaxOnEdgeRetentionCleanup(env);
    return {
      ...retention,
      maintenance_non_fatal: true,
      warning: retention.ok ? null : 'retention_cleanup_failed_non_fatal',
    };
  } catch (error) {
    return {
      ok: false,
      maintenance_non_fatal: true,
      warning: 'retention_cleanup_failed_non_fatal',
      error: error?.message || String(error),
      no_fake_value: true,
    };
  }
...
export async function runWaxOnEdgeScheduledSync(env, cron = '') {
  if (!env.DB) return { ok: false, error: 'DB binding is not configured' };
  const freeSafeMode = waxonedgeFreeSafeMode(env);
  if (cron === 'waxonedge-retention-cleanup') {
    const retention = await runWaxOnEdgeRetentionCleanup(env);
    return { ok: retention.ok, retention_cleanup: true, free_safe_mode: freeSafeMode, retention };
  }
  ...
  if ((isMinuteCron || isQuarterHourCron) && freeSafeMode) {
    ...
    tasks.push((async () => {
      ...
      return { ok: alcor.ok && core.ok && nefty.ok && pinned.ok && tradeBackfill.ok && holders.ok && aggregates.ok, syncCycleId, alcor, core, nefty, pinned, tradeBackfill, holders, aggregates, retention, membershipSnapshot };
    })());
