-- Immutable Moonpet gameplay authority: beta runtime clean cutover.
-- Permanent Telegram/account identity, wallet linkage, NFT ownership and D1
-- migration evidence are untouched. Beta gameplay rows are deliberately reset
-- rather than assigning historical account-only activity to today's selector.

-- Delete child/evidence rows before their parents. These tables contain beta
-- gameplay state only; the clean reset makes every surviving post-064 record
-- provably pet-authoritative.
...
DELETE FROM telegram_pet_cosmetic_unlocks;
...
CREATE TABLE telegram_pet_identity_events (
  event_id TEXT PRIMARY KEY, telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, event_key TEXT NOT NULL,
...
CREATE TABLE telegram_pet_personality_traits (
  pet_id TEXT NOT NULL, telegram_id TEXT NOT NULL, trait_id TEXT NOT NULL, progress INTEGER NOT NULL DEFAULT 0 CHECK(progress>=0),
...
CREATE TABLE telegram_pet_memories (
  pet_id TEXT NOT NULL, telegram_id TEXT NOT NULL, first_adoption_at DATETIME, first_run_at DATETIME, first_extraction_at DATETIME,
...
CREATE TABLE telegram_pet_boss_victories (pet_id TEXT NOT NULL, telegram_id TEXT NOT NULL, boss_id TEXT NOT NULL, victories INTEGER NOT NULL DEFAULT 0 CHECK(victories>=0), updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
CREATE TABLE telegram_pet_run_history (
  telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, runs_completed INTEGER NOT NULL DEFAULT 0 CHECK(runs_completed>=0),
...
-- Stable view names remain for read clients; after the cutover they expose only pet-authoritative rows.
...
-- Phase 5 progression and live-system state are rebuilt empty so account-keyed uniqueness cannot collapse distinct pets.
...
CREATE TABLE telegram_pet_progression_state (
  telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, care_xp INTEGER NOT NULL DEFAULT 0, training_xp INTEGER NOT NULL DEFAULT 0,
...
CREATE TABLE telegram_pet_runtime_events (id TEXT PRIMARY KEY, telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, event_key TEXT NOT NULL, action TEXT NOT NULL, payload_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
CREATE TABLE telegram_pet_event_chain_progress (telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, chain_key TEXT NOT NULL, step_index INTEGER NOT NULL DEFAULT 0, completed_cycles INTEGER NOT NULL DEFAULT 0, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
CREATE TABLE telegram_pet_seasonal_boss_progress (telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, season_key TEXT NOT NULL, boss_key TEXT NOT NULL, damage INTEGER NOT NULL DEFAULT 0, defeated_at TEXT, reward_claimed_at TEXT, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
CREATE TABLE telegram_pet_cosmetic_unlocks (telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, cosmetic_key TEXT NOT NULL, quantity INTEGER NOT NULL DEFAULT 1, unlocked_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
CREATE TABLE telegram_pet_equipment_progression (
  telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, item_key TEXT NOT NULL, slot TEXT NOT NULL, item_level INTEGER NOT NULL DEFAULT 1 CHECK(item_level BETWEEN 1 AND 10),
...
CREATE TABLE telegram_pet_equipment_events (id TEXT PRIMARY KEY, telegram_id TEXT NOT NULL, pet_id TEXT NOT NULL, item_key TEXT NOT NULL, action TEXT NOT NULL, event_key TEXT NOT NULL, item_xp_awarded INTEGER NOT NULL DEFAULT 0, mastery_xp_awarded INTEGER NOT NULL DEFAULT 0, metadata_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY(pet_id) REFERENCES telegram_pet_instances(pet_id));
...
-- INSERT guards make missing authority impossible after the beta reset.
