...
check(csp.includes('.csp-live-row{') && csp.includes('font-size:.64rem') && csp.includes('line-height:1.16') && csp.includes('padding:2px 0'), 'compact live rows use reduced typography and tight vertical spacing');
check(csp.includes('.csp-ops-row{') && csp.includes('font-size:.64rem') && csp.includes('line-height:1.16') && csp.includes('padding:2px 0'), 'compact ops rows use reduced typography and tight vertical spacing');
check(csp.includes('.csp-live-row-val{') && csp.includes('min-width:0') && csp.includes('overflow:hidden') && csp.includes('text-overflow:ellipsis') && csp.includes('white-space:nowrap'), 'live row values enforce min-width:0 + nowrap ellipsis to prevent vertical wrapping');
check(csp.includes('.csp-ops-val{') && csp.includes('min-width:0') && csp.includes('overflow:hidden') && csp.includes('text-overflow:ellipsis') && csp.includes('white-space:nowrap'), 'ops row values enforce min-width:0 + nowrap ellipsis to prevent vertical wrapping');
check(csp.includes('.csp-feed-text{') && csp.includes('font-size:.62rem') && csp.includes('white-space:nowrap') && csp.includes('text-overflow:ellipsis'), 'latest/feed text uses compact one-line ellipsis styling');
check(csp.includes('.csp-mission-row{') && csp.includes('white-space:nowrap') && csp.includes('padding:2px 0') && csp.includes('font-size:.66rem'), 'mission rows are compact single-line entries');
...
check(csp.includes('.csp-wtf-badge{') && csp.includes('font-size:.58rem') && csp.includes('padding:2px 8px'), 'WTF badge typography is compact for right-rail layout');
check(csp.includes('.csp-missed-badge{') && csp.includes('font-size:.58rem') && csp.includes('padding:2px 8px'), 'MISSED badge typography is compact while preserving warning styling');
check(functionBlock(csp, 'blocktopiaAccessHTML').includes("return '<span class=\"csp-val-good\">Unlocked</span>';") && functionBlock(csp, 'blocktopiaAccessHTML').includes("'Locked ' + esc(String(arcadeXp)) + '/' + requiredXp"), 'Block Topia row uses compact right-rail copy (Unlocked / Locked x/y)');
check(opsBlock.includes('>Battle<') && opsBlock.includes('>Open<') && !opsBlock.includes('Battle Chamber') && !opsBlock.includes('Open Battle Chamber'), 'Battle Chamber row uses compact Battle/Open copy in the narrow right rail');
check(wtfSectionBlock.includes('>Ready<') && wtfSectionBlock.includes('>Play<') && wtfSectionBlock.includes('>Open<') && !wtfSectionBlock.includes('Get Ready') && !wtfSectionBlock.includes('Play Arcade') && !wtfSectionBlock.includes('Open Arcade'), 'Daily WTF action labels are compact (Ready/Play/Open)');
check(missedSectionBlock.includes('csp-live-row-val csp-live-row-val--warn') && csp.includes('.csp-live-row-val{') && csp.includes('white-space:nowrap'), 'Missed values use compact row-value nowrap styling so numbers cannot stack vertically');
