...
check(las.includes('normalizeEmergencyState') && las.includes("fetch(base + '/wtf/events/today'") && las.includes('maybeKickEmergencyWtfRecovery'), 'right-rail can directly recover real WTF feed data when shared state is temporarily unavailable');
check(las.includes("if ((!state || state.status === 'error') && _singleton.wtfEmergencyState)") && las.includes('state = _singleton.wtfEmergencyState'), 'right-rail applies recovered real-feed state before rendering unavailable card');
...
check(las.includes("if (state.status === 'error') {") && las.includes('maybeKickEmergencyWtfRecovery();') && las.includes('if ((!state || state.status === \'error\') && _singleton.wtfEmergencyState)'), 'unavailable card is recoverable and does not persist when recovered next_event state exists');
