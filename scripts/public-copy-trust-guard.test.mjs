...
const FORBIDDEN_COPY = [
  ...
  { label: 'automatic-hodl-warrior', pattern: /\b(?:anyone|any|every)\s+(?:crypto|nft)\s+holders?\s+(?:automatically\s+)?(?:becomes|become|are)\s+a\s+HODL\s+Warrior\b/i },
  { label: 'automatic-hodl-warrior', pattern: /\b(?:crypto|nft)\s+holders?\s+(?:automatically\s+)?(?:becomes|become|are)\s+a\s+HODL\s+Warrior\b/i },
