...
const FORBIDDEN_COPY = [
  ...
  { label: 'automatic-hodl-warrior', pattern: /\b(every|anyone|any)\s+(qualifying\s+unique\s+1\/1\s+|crypto\s+|nft\s+)?(holder|holders)\s+(automatically\s+)?(becomes|become|are)\s+a\s+HODL\s+Warrior\b/i },
