...
assert.match(battleLayer, /function movePageLikeIntoMissions\(\)[\s\S]*\.page-like-widget[\s\S]*\.battle-engagement-deck--collection \.battle-shell--missions \.battle-shell-inner[\s\S]*mission-like-row/, 'collection article like widget should move into Daily Missions without duplicating hooks');
