...
const deployPagesWorkflow = readFileSync('.github/workflows/pages.yml', 'utf8');
...
assert.match(btqmLivePage, /mountGame\(\{\s*root:\s*document\.querySelector\(['"]\.game-card['"]\)\s*,\s*bootstrap:\s*bootstrapBlockTopiaQuestMaze[\s\S]*?requireCompetitiveGate:\s*true[\s\S]*?gameId:\s*['"]block-topia-quest-maze['"][\s\S]*?\}\)/, 'BTQM live page should mount the maintained BTQM bootstrap through the shared competitive gate');
