...
const backupDir = path.join(root, ".tmp", "stage0-promotion-contract-backup");
...
try {
  ...
  for (const role of stageRoles) {
    ...
    assert.equal(asset.review_status, "mechanical_validation_passed_pending_visual_review", `${role} must stay pending visual review`);
  }
...
assert.equal(stage0ManifestStatus.stdout.trim(), "", "promotion contract test must restore tracked Stage-0 manifest");
