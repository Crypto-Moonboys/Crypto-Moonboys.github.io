...
const publicPairRows = [
  ...
  {
    source: 'swap.nefty',
    pair_id: 'SUSPICIOUS_WPIXAL_WUF',
    token_a_contract: 'pixeljourney',
    token_a_symbol: 'WPIXAL',
    token_b_contract: 'wuffi',
    token_b_symbol: 'WUF',
    reserve_a: '1000',
    reserve_b: '5',
    liquidity_wax: '75000000',
    liquidity_usd: '450000',
    volume_24h_wax: null,
    updated_at: '2026-06-22T00:04:30.000Z',
  },
  {
    source: 'swap.alcor',
    pair_id: 'LARGE_WAX_DIRECT_WUF',
    token_a_contract: 'wuffi',
    token_a_symbol: 'WUF',
    token_b_contract: 'eosio.token',
    token_b_symbol: 'WAX',
  ...
  return {
    prepare(sql) {
      return {
        ...
        first() {
          if (this.sql.includes('FROM waxonedge_token_stats')) {
            ...
            if (contract === 'wuffi' && symbol === 'WUF') {
              return Promise.resolve({
                direct_waxcash_pair_liquidity_wax: '5849755.08493458',
                selected_pair_source: 'swap.taco',
                selected_pair_id: 'WAXCASH_WUF_VERIFIED',
                updated_at: '2026-06-22T00:00:00.000Z',
              });
            }
          ...
          if (this.sql.includes('COUNT(*) AS count') && this.sql.includes('FROM waxonedge_pairs')) {
            ...
            const hasGenericTokenPageLiquidityProof = this.sql.includes("CAST(COALESCE(volume_24h_wax, '0') AS NUMERIC) > 0");
            return Promise.resolve({
              count: targetRows.filter((row) =>
                rowHasBlockedContract(row) ||
                rowHasImpossibleLiquidity(row) ||
                (hasGenericTokenPageLiquidityProof && rowFailsGenericTokenPageLiquidityProof(row))
              ).length,
            });
          }
          return Promise.resolve(null);
...
ok('/api/waxonedge/token-page/wuffi/WUF ranks WAXCASH/WUF by verified display liquidity when available',
  wufTokenPage.pairs[0]?.pair_id === 'WAXCASH_WUF_VERIFIED' &&
  wufTokenPage.pairs[0]?.display_liquidity_wax === '5849755.08493458' &&
...
ok('/api/waxonedge/waxcash-analytics WAXCASH pair loader uses the same Worker-side public feed policy',
  waxcashPairRows.length === 2 &&
  waxcashPairRows.some((row) => row.pair_id === 'GOODWAXCASH') &&
  waxcashPairRows.some((row) => row.pair_id === 'WAXCASH_WUF_VERIFIED') &&
