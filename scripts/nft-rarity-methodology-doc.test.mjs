...
assert.match(doc, /Template Rarity Ranking compares template editions/, 'doc should name Template Rarity Ranking');
assert.match(doc, /Asset Version Ranking compares exact live NFTs/, 'doc should name Asset Version Ranking');
assert.match(doc, /asset_final_score =[\s\S]*normalized_template_final_score \* 60[\s\S]*original_mint_score \* 20[\s\S]*surviving_mint_rank_score \* 20/, 'doc should include the Asset Version Ranking formula');
assert.match(doc, /built on top of template rank/i, 'doc should say asset rank is built on template rank');
assert.match(doc, /Burned assets are excluded from Asset Version Ranking/i, 'doc should say burned assets are excluded from asset ranking');
...
assert.match(doc, /marketplace price, floor, sales, listings, market cap, and volume are excluded from asset ranking/i, 'doc should exclude market data from asset ranking');
...
assert.match(doc, /asset-state cache[\s\S]*surviving mint ranks[\s\S]*holder leaderboard[\s\S]*Asset Version Ranking/i, 'doc should list NoBallGames asset-state and Asset Version Ranking outputs');
