...
try {
  ...
  const returned = await page.evaluate(() => window.MoonpetBotArtRenderer.selectMoonpetBot({ speciesId: "comet_gecko", speciesName: "TUBBY" }));
  assert.equal(returned.resolvedBot, "TUBBY");
  assert.equal(returned.fallbackUsed, false);

  await waitForPack(page, "TUBBY");
  await fs.mkdir(OUTPUT, { recursive: true });
  await page.screenshot({ path: path.join(OUTPUT, "tubby-mobile-390x844.png"), fullPage: false });
  console.log(JSON.stringify({ botty: "pass", tubbyActions: tubby.renders.length, unknownFallback: unknown.resolvedBot, switchBack: returned.resolvedBot, mobile: "390x844" }));
} finally {
  await browser.close();
