...
check(read('js/components/live-activity-summary.js').includes('normalizeEmergencyState') && read('js/components/live-activity-summary.js').includes('getSignedAuthForEmergencyRecovery') && read('js/components/live-activity-summary.js').includes('maybeKickEmergencyWtfRecovery'), 'right-rail can directly recover from real /wtf/events/today payloads');
check(read('js/components/live-activity-summary.js').includes("method: 'POST'") && read('js/components/live-activity-summary.js').includes("body: JSON.stringify({ telegram_auth: auth })") && read('js/components/live-activity-summary.js').includes("fetch(base + '/wtf/events/today', requestOptions)") && read('js/components/live-activity-summary.js').includes('var requestOptions = auth'), 'right-rail emergency recovery mirrors linked POST auth and unlinked GET patterns');
check(read('js/components/live-activity-summary.js').includes("if ((!state || state.status === 'error') && emergencyState)") && read('js/components/live-activity-summary.js').includes('getEmergencyStateIfUsable') && read('js/components/live-activity-summary.js').includes('WTF_EMERGENCY_STATE_TTL_MS'), 'right-rail keeps recovered emergency state while primary remains errored, with bounded TTL');
check(read('js/components/live-activity-summary.js').includes('hasUsablePrimaryWtfState(getWtfState())') && read('js/components/live-activity-summary.js').includes('_singleton.wtfEmergencyState = null;') && read('js/components/live-activity-summary.js').includes('_singleton.wtfEmergencyRecoveredAt = 0;'), 'right-rail clears emergency state after primary feed recovers');
...
check(read('js/components/live-activity-summary.js').includes('Daily WTF: Loading signal') && read('js/components/live-activity-summary.js').includes('Daily WTF: Signal unavailable'), 'right-rail fallback copy stays compact during reconnect and unavailable states');
