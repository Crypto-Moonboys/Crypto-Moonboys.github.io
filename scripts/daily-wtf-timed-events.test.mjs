...
check(read('js/components/live-activity-summary.js').includes('normalizeEmergencyState') && read('js/components/live-activity-summary.js').includes("fetch(base + '/wtf/events/today'") && read('js/components/live-activity-summary.js').includes('maybeKickEmergencyWtfRecovery'), 'right-rail can directly recover from real /wtf/events/today payloads');
check(read('js/components/live-activity-summary.js').includes("if ((!state || state.status === 'error') && _singleton.wtfEmergencyState)") && read('js/components/live-activity-summary.js').includes('state = _singleton.wtfEmergencyState'), 'right-rail clears previous error/fallback display once recovered next_event state exists');
