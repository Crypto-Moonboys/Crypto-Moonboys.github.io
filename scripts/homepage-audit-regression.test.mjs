...
assert.match(widgets, /\^\\\/\(\?!\\\/\)\//, 'safeHref must reject protocol-relative URLs');
...
assert.ok(deployWorkflow.includes('node scripts/generate-sitemap.js'), 'deployment must regenerate sitemap.xml');
...
assert.ok(sitemap.trim().endsWith('</urlset>'), 'committed sitemap must be validly closed');
