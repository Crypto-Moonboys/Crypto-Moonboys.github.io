...
let resolveLinkedAuthRequiredFetch;
const authRequiredToLinkedCalls = [];
const restoredAuthPayload = { id: 12345, hash: 'restored-hash', auth_date: 1782782222 };
const authRequiredToLinkedRuntime = createRuntime({
  linked: true,
  auth: null,
  fetchImpl(url, options = {}) {
    authRequiredToLinkedCalls.push({ url: String(url), options });
    if ((options.method || 'GET') === 'GET') {
      return new Promise((resolve) => {
        resolveLinkedAuthRequiredFetch = () => resolve(response(dailyPayload()));
      });
    }
    return response(dailyPayload({ identity: { linked: true, auth_mode: 'telegram_verified' } }));
  },
});
const linkedAuthRequiredInFlight = authRequiredToLinkedRuntime.window.MOONBOYS_DAILY_LOOP.refresh({ force: true });
await waitForCalls(authRequiredToLinkedCalls, 1);
authRequiredToLinkedRuntime.setAuth(restoredAuthPayload);
const linkedFreshAfterAuthRequired = authRequiredToLinkedRuntime.window.MOONBOYS_DAILY_LOOP.refresh({ force: true });
assert.notEqual(linkedAuthRequiredInFlight, linkedFreshAfterAuthRequired, 'linked:auth_required in-flight GET must not be reused for fresh-auth linked POST');
await linkedFreshAfterAuthRequired;
resolveLinkedAuthRequiredFetch();
await linkedAuthRequiredInFlight;
assert.equal(authRequiredToLinkedCalls.length, 2, 'fresh auth during auth_required fetch starts a separate linked request');
assert.equal(authRequiredToLinkedCalls[0].options.method, 'GET');
assert.equal(authRequiredToLinkedCalls[1].options.method, 'POST');
assert.equal(authRequiredToLinkedCalls[1].url.includes('?'), false, 'fresh linked auth after auth_required race must not use query strings');
assert.deepEqual(JSON.parse(authRequiredToLinkedCalls[1].options.body), { telegram_auth: restoredAuthPayload });
...
const authRestoreCacheCalls = [];
const authRestorePayload = { id: 12345, hash: 'cache-restore-hash', auth_date: 1782783333 };
const authRestoreRuntime = createRuntime({
  linked: true,
  auth: null,
  fetchImpl(url, options = {}) {
    authRestoreCacheCalls.push({ url: String(url), options });
    if ((options.method || 'GET') === 'POST') {
      return response(dailyPayload({ identity: { linked: true, auth_mode: 'telegram_verified' } }));
    }
    return response(dailyPayload());
...
const cachedAuthRequiredState = await authRestoreRuntime.window.MOONBOYS_DAILY_LOOP.getState();
assert.equal(cachedAuthRequiredState.identity.auth_mode, 'auth_required');
assert.equal(authRestoreCacheCalls.length, 1);
assert.equal(authRestoreCacheCalls[0].options.method, 'GET');
authRestoreRuntime.setAuth(authRestorePayload);
const restoredLinkedState = await authRestoreRuntime.window.MOONBOYS_DAILY_LOOP.getState();
assert.equal(restoredLinkedState.identity.auth_mode, 'telegram_verified');
assert.equal(authRestoreCacheCalls.length, 2, 'cached auth_required state must not block a later linked POST after auth restore');
assert.equal(authRestoreCacheCalls[1].options.method, 'POST');
...
assert.ok(cspSrc.includes('shared.dailyLoopOwnsFaction = true'), 'right rail marks daily-loop-owned faction fields');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsDailyMissions = true'), 'right rail marks daily-loop-owned daily mission fields');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsMissed = true'), 'right rail marks daily-loop-owned missed fields');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsWtf = true'), 'right rail marks daily-loop-owned WTF fields');
assert.ok(cspSrc.includes('if (contribution.pending)') && cspSrc.includes('if (!shared.dailyLoopOwnsFaction)'), 'legacy contribution fetch is guarded by daily-loop faction ownership');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsDailyMissions ? shared.dailyOpsStatus : getDailyOpsStatus'), 'legacy daily mission status cannot overwrite daily-loop mission status');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsMissed ? shared.missedXp : missedXpAllTime'), 'legacy missed values cannot overwrite daily-loop missed values');
assert.ok(cspSrc.includes('shared.dailyLoopOwnsWtf ? shared.dailyWtfStatusDisplay : getDailyWtfSignalStatus'), 'legacy WTF status cannot overwrite daily-loop WTF truth');
